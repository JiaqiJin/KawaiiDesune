...
namespace Kawaii
{
    ...
    bool WindowManager::Initialize(const WindowState& windowState)
    {
        ...
        if (ms_WindowClass == 0)
        {
            ...
            windowClassDesc.lpszClassName = KAWAII_WINDOW_CLASS_NAME;

            ms_WindowClass = RegisterClassExW(&windowClassDesc);
            assert(ms_WindowClass != 0);
        }
    ...
    {
        KAWAII_SET_CURRENT_THREAD_NAME(L"Window Thread");
    ...
    void WindowManager::InitializeWindow(const WindowState& windowState)
    {
        ...
        m_HWND = CreateWindowExW(
            NULL, KAWAII_WINDOW_CLASS_NAME, KAWAII_GAME_NAME, windowStyle, windowState.x, windowState.y,
