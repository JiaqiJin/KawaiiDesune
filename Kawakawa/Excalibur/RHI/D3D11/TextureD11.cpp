...
#include "../../Utils/ImageParser.h"

Excalibur::TextureD11::TextureD11(const std::string& filepath)
{
	Initialize(filepath);
}

Excalibur::TextureD11::~TextureD11()
{
	Finialize();
}

void Excalibur::TextureD11::Initialize(const std::string& filepath) noexcept
{
	HRESULT result;
	auto mgrd11 = (GraphicsMgrD11*)GApp->mGraphicsManager;
	int width, height, channels;
	auto data = Excalibur::ImageParser::Parse(filepath, &width, &height, &channels, 4);
...
void Excalibur::TextureD11::Finialize() noexcept {
	if (mView) {
		mView->Release();
	}
	if (mTexture) {
