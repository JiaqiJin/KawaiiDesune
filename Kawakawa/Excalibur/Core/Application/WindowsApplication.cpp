...
namespace Excalibur
{
	...
	LRESULT Excalibur::WindowsApplication::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
	...
	int WindowsApplication::Initialize() {
		...
		m_MemoryMgr = new MemoryManager();
		m_MemoryMgr->Initialize();

		mUseOpengl = true;

		LoadWGL();
		CreateGLContext();
		m_GraphicsManager = new GraphicsMgrGL();
		auto mgr = (GraphicsMgrGL*)m_GraphicsManager;
		mgr->Initialize();
	...
	HWND Excalibur::WindowsApplication::GetWindowsHandler()
	{
		return mHWND;
	}

	void Excalibur::WindowsApplication::CreateMainWindow()
	{
