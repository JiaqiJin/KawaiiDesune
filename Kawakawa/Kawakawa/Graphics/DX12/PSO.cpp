...
namespace Kawaii::Graphics::backend::DX12
{
	...
	void GraphicsPSO::SetSampleMask(unsigned mask)
	{
		m_PSODesc.SampleMask = mask;
	}

	void GraphicsPSO::SetRenderTargetFormats(const std::vector<DXGI_FORMAT>& rtv_formats,
		DXGI_FORMAT dsv_format, unsigned msaa_count, unsigned msaa_quality)
	{
		for (int i = 0; i < rtv_formats.size(); i++) 
			m_PSODesc.RTVFormats[i] = rtv_formats[i];

		for (int i = rtv_formats.size(); i < m_PSODesc.NumRenderTargets; i++)
	...
	// -------------------- Compute ----------------------------
	ComputePSO::ComputePSO(std::string_view name)
		: PSO(name)
	{
